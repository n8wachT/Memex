...
import * as overviewActs from '../actions'
import * as overview from '../selectors'
...
export const search: (args: any) => Thunk = (
    ...
    dispatch(overviewActs.setLoading(true))
    ...
    if (overwrite) {
        dispatch(overviewActs.resetPage())
    }

    if (/thank you/i.test(query)) {
        return dispatch(overviewActs.easter())
    }
    ...
    const searchParams = {
        ...
        skip: overview.resultsSkip(state),
    ...
    try {
        ...
        dispatch(overviewActs.updateSearchResult({ overwrite, searchResult }))
        ...
        dispatch(overviewActs.setLoading(false))
    }
